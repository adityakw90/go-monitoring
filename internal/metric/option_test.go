...
func TestMetric_Option_WithServiceName(t *testing.T) {
	tests := []struct {
		name        string
		serviceName string
...
func TestMetric_Option_WithEnvironment(t *testing.T) {
	tests := []struct {
		name        string
		environment string
		checkFunc   func(t *testing.T, opts *Options)
...
func TestMetric_Option_WithInstance(t *testing.T) {
	tests := []struct {
		name         string
		instanceName string
		instanceHost string
...
func TestMetric_Option_WithProvider(t *testing.T) {
	tests := []struct {
		name      string
		provider  string
		host      string
		...
		},
		{
			...
			checkFunc: func(t *testing.T, opts *Options) {
				...
				if opts.ProviderPort != 4319 {
					t.Errorf("WithProvider() set ProviderPort = %v, want %v", opts.ProviderPort, 4319)
				}
			},
		},
		{
...
func TestMetric_Option_WithInterval(t *testing.T) {
	tests := []struct {
		name      string
	...
	}{
		{
			...
			checkFunc: func(t *testing.T, opts *Options) {
				if opts.Interval != 1*time.Second {
					t.Errorf("WithInterval() set Interval = %v, want %v", opts.Interval, 1*time.Second)
				}
			},
		},
		{
			name:     "set interval 30 seconds",
			interval: 30 * time.Second,
			checkFunc: func(t *testing.T, opts *Options) {
				if opts.Interval != 30*time.Second {
					t.Errorf("WithInterval() set Interval = %v, want %v", opts.Interval, 30*time.Second)
				}
			},
		},
		{
			...
			checkFunc: func(t *testing.T, opts *Options) {
				if opts.Interval != 60*time.Second {
					t.Errorf("WithInterval() set Interval = %v, want %v", opts.Interval, 60*time.Second)
				}
			},
		},
		{
			name:     "set interval 100 milliseconds",
			interval: 100 * time.Millisecond,
			checkFunc: func(t *testing.T, opts *Options) {
				if opts.Interval != 100*time.Millisecond {
					t.Errorf("WithInterval() set Interval = %v, want %v", opts.Interval, 100*time.Millisecond)
				}
			},
		},
		{
			name:     "set interval 1 minute",
			interval: 1 * time.Minute,
			checkFunc: func(t *testing.T, opts *Options) {
				if opts.Interval != 1*time.Minute {
					t.Errorf("WithInterval() set Interval = %v, want %v", opts.Interval, 1*time.Minute)
				}
			},
		},
		{
			name:     "set interval 5 minutes",
			interval: 5 * time.Minute,
			checkFunc: func(t *testing.T, opts *Options) {
				if opts.Interval != 5*time.Minute {
					t.Errorf("WithInterval() set Interval = %v, want %v", opts.Interval, 5*time.Minute)
				}
			},
		},
	}
...
func TestMetric_Option_WithInsecure(t *testing.T) {
	tests := []struct {
		name      string
		insecure  bool
		checkFunc func(t *testing.T, opts *Options)
...
func TestMetric_Option_MultipleOptions(t *testing.T) {
	tests := []struct {
		name      string
		opts      []Option
		checkFunc func(t *testing.T, opts *Options)
	}{
		{
			...
			opts: []Option{
				...
				WithProvider("otlp", "collector.example.com", 4318),
			...
			checkFunc: func(t *testing.T, opts *Options) {
				...
				if opts.ProviderPort != 4318 {
					t.Errorf("ProviderPort = %v, want %v", opts.ProviderPort, 4318)
				}
				if opts.Interval != 30*time.Second {
					t.Errorf("Interval = %v, want %v", opts.Interval, 30*time.Second)
				}
				if opts.Insecure != true {
		...
		},
		{
			...
			checkFunc: func(t *testing.T, opts *Options) {
				...
				if opts.Interval != 60*time.Second {
					t.Errorf("Interval = %v, want %v", opts.Interval, 60*time.Second)
				}
				if opts.Insecure != false {
		...
		},
		{
			...
			checkFunc: func(t *testing.T, opts *Options) {
				...
				if opts.Interval != 20*time.Second {
					t.Errorf("Interval = %v, want %v", opts.Interval, 20*time.Second)
				}
			},
		},
	}
